...
public:
	...
	NodB(string k3)
	{
		...
		g.open("B resources\\to send.txt");
		f.open("B resources\\received.txt");
		getline(g, plaintext);
	}
	...
	{
		return encryptECB("B: Message REceived<<<------");
	}
	...
	/*string eightBlocksSent()
	{
		return encryptECB("8 blocks sent");
	}*/
	string sendMessageECB()
	{
		//cout << endl << encryptECB(plaintext) << endl<<plaintext;
